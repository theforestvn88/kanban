    ...
    class Mapper
        def kanban(name, board: "board", list: "list", card: "card")
            ...
            namespace "kanban7", as: "" do
                ...
                get "/:name/#{list}s/:#{list}_id/edit", to: 'lists#edit', as: "#{name}_kanban_edit_#{list}", :defaults => { :name => name }
                patch "/:name/#{list}s/:#{list}_id", to: 'lists#update', as: "#{name}_kanban_update_#{list}", :defaults => { :name => name }
                delete "/:name/#{list}s/:#{list}_id", to: 'lists#destroy', as: "#{name}_kanban_delete_#{list}", :defaults => { :name => name }
                
                get "/:name/#{card}s/:#{card}_id", to: 'cards#show', as: "#{name}_kanban_show_#{card}", :defaults => { :name => name }
